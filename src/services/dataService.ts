 ...
 *
 * Current implementation: merges language-neutral base JSON with locale-specific i18n JSON.
 * Future migration: replace the merge functions with async API calls that return the same types.
 */
import type { Person, Company, Project, Skill, Tag } from '@/types'

// Base (language-neutral) data
import basePersonRaw   from '@/data/base/person.json'
...
const i18nMap: Record<string, I18nData> = {
  ko: koData,
  en: enData,
  ja: jaData
}
...
export function getPerson(locale: string): Person {
  ...
  return { ...basePersonRaw, ...i18n.person } as Person
}
